...
//директивы, пайпы, компоненты не шарятся между модулями без импортов, сервисы шарятся (верхний модуль шарит на чайлдов), однако нельзя в модуль из роутингов шарить общий сервис, так как каждый из них создаст свой экземпляр.
